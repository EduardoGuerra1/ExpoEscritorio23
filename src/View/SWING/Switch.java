...
public class Switch extends JComponent {
    ...
    private List<EventoSwitch> events;
    private Color color;
    ...
    private void startAnimation() {
        if (!animator.isRunning()) {
            if (isSelected()) {
                ...
                colorTarget = new PropertySetter(Switch.this, "color", tema.mainColor, new Color(80, 80, 80));
                animator.addTarget(colorTarget);
            } else {
                animator.removeTarget(colorTarget);
                colorTarget = new PropertySetter(Switch.this, "color", new Color(80, 80, 80), tema.mainColor);
                animator.addTarget(colorTarget);
            }
            animator.start();
            runEvent(!selected);
    ...
    private void runEvent(boolean selected) {
        for (EventoSwitch event : events) {
            event.onSelected(selected);
        }
    }

    public void addEventSelected(EventoSwitch event) {
        events.add(event);
    }
